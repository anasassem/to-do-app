...
class NotificationServices {
  ...
  scheduledNotification(int hour, int minutes, Task task, int id) async {
  ...
  tz.TZDateTime _nextInstanceOfTenAM(
    ...
    final fd = tz.TZDateTime.from(formattedDate, tz.local);
...
//Older IOS
  Future onDidReceiveLocalNotification(
    ...
    await Get.dialog(Text(body!));
  }

  void _configureSelectNotificationSubject() {
    selectNotificationSubject.stream.listen((String payload) async {
      debugPrint('My payload is ' + payload);
      await Get.toNamed(Routes.NOTIFICATION, arguments: {'payload': payload});
    });
  }
}
